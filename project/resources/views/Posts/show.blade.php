...
<div class="container container-comment mt-5">
    <div class="row mb-1">
        ...
        <div class="col-2 offset-2">
            <button class="text-danger report btn fieldset"><i class="fas fa-exclamation"></i> Report</button>
        </div>
    </div>
    <div class="row mb-1">
        <div class="col-6 textReport offset-6"  style="display:none">
            <form method="POST" action="{{route('reports.store',$post->id)}}" class="w-100">
                <input type="hidden" name="_token" value="{{csrf_token()}}">
                <div class="form-group w-100">
                    <textarea class="form-control" id="exampleInputEmail1" name="content" aria-describedby="emailHelp"
                        placeholder="Your report"></textarea>
                </div>
                <input type="hidden" class="form-control" name="post_id" value={{$post->id}}>
                <input type="hidden" class="form-control" name="user_id" value={{$user->id}}>
                <div class="d-flex justify-content-end">
                    <button type="submit" class="btn btn-success" name="">Report</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    ...
    $(document).ready(function () {
        $(".report").click(function () {
            if (i == 0) {
                $(".textReport").css("display", "flex");
                i = 1;
            } else {
                $(".textReport").css("display", "none");
                i = 0;
            }
        });
    });
